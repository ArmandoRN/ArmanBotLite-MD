...
let chatDisplay = m.chat.includes('@s.whatsapp.net') ? chalk.cyanBright(m.chat) : `${chalk.greenBright(m.chat)} ${chalk.greenBright(chat ? '⇢ ' + chat : '')}`

console.log(`${chalk.bold.greenBright('✪▸▸')} ${chalk.bold.magenta('%s')} ${chalk.bold.greenBright('◂◂✪')} 
┊» ${chalk.black(chalk.whiteBright('%s'))} ${chalk.black(chalk.whiteBright('%s'))}
┊» ${chalk.magenta('%s (%s %sB)')} || ${chalk.blue(vs + ' 𓃠')} || ${chalk.black(chalk.yellowBright('[%s]'))}
┊» ${chalk.cyan(sender)} ${chalk.white(`"${user.name ? user.name : ''}"`)} 
┊» ${chatDisplay}
╰╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌•`,
 
me + ' ⇢ ' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : '(AB - SUB BOT)'}`,
